$(function(){
	// 信息流
	var get_href , token, activityId = 'c59bb4e8831211eb8d48506b4b4a9882', partId = getCookie('partId') || '', code0120, phone0120,name0120, timer_code, getCookie0120 = getCookie('_userid') || '';
    ...
    $(document).on('click', '.btnlogin20', function () {
	    ...
	    if (getCookie0120.length == 32) {//已登录
			...
			window.location.href="https://m.zbgedu.com/api/PayAccounting/pay?payId="+wxug+"&shopId=935"
	    }else {//未登录
	    ...
	    $.ajax({
	      ...
	      success: function (res) {
	        ...
	        if (res.state == 'success') {//登录成功
				// 转换代码  convert_id
				meteor.track("shopping", {convert_id: "1696463761775627"})
				reset();
				var wxug = isWeiXin()?'3':'2';
				window.location.href="https://m.zbgedu.com/api/PayAccounting/pay?payId="+wxug+"&shopId=935"
